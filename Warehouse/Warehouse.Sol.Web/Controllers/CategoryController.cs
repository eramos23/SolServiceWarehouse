...
namespace Warehouse.Sol.Web.Controllers
{
    ...
    public class CategoryController : BaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<List<ProductoCategoriaDto>>))]
        ...
        {
            DataQuery<List<ProductoCategoriaDto>> result;

            var list = await _service.GetAllAsync(filterDto);
            if (list != null)
            {
                result = HelperStatus.ResponseHelper(this._mapper.Map<List<ProductoCategoriaDto>>(list), HttpStatusCode.OK);
                return Ok(result);
            }
            result = HelperStatus.ResponseHelper(new List<ProductoCategoriaDto>(), HttpStatusCode.NotFound, Messages.Result_Empty);
            return NotFound(result);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<ProductoCategoriaDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(DataQuery<object>))]
        ...
        public async Task<ActionResult> GetById(string id)
        {
            ...
            DataQuery<ProductoCategoriaDto> result;

            var resultService = await _service.GetByIdAsync(new Guid(id));

            if (resultService != null)
            {
                result = HelperStatus.ResponseHelper(_mapper.Map<ProductoCategoriaDto>(resultService), HttpStatusCode.OK);
                return Ok(result);
            }
        ...
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(DataQuery<ProductoCategoriaDto>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] PostProductoCategoriaDto dto)
        {
            if (dto == null) return BadRequest(HelperStatus.ResponseHelper(false, HttpStatusCode.BadRequest, Messages.Result_BadRequest));
            DataQuery<ProductoCategoriaDto> result;
            var category = _mapper.Map<ProductoCategoria>(dto);
            ...
            if (resultService)
            {
                ...
                result = HelperStatus.ResponseHelper(_mapper.Map<ProductoCategoriaDto>(providerCrearted), HttpStatusCode.Created, Messages.Result_Created);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(string id, [FromBody] PutProductoCategoriaDto dto)
        {
            ...
            var categoryDto = _mapper.Map<ProductoCategoria>(dto);
            var resultService = await _service.UpdateAsync(new Guid(id), categoryDto);
