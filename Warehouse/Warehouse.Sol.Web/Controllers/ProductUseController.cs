...
namespace Warehouse.Sol.Web.Controllers
{
    ...
    public class ProductUseController : BaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<List<ProductoUsoDto>>))]
        ...
        {
            DataQuery<List<ProductoUsoDto>> result;

            var list = await _service.GetAllAsync(filterDto);
            if (list != null)
            {
                result = HelperStatus.ResponseHelper(this._mapper.Map<List<ProductoUsoDto>>(list), HttpStatusCode.OK);
                return Ok(result);
            }
            result = HelperStatus.ResponseHelper(new List<ProductoUsoDto>(), HttpStatusCode.NotFound, Messages.Result_Empty);
            return NotFound(result);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<ProductoUsoDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(DataQuery<object>))]
        ...
        public async Task<ActionResult> GetById(string id)
        {
            ...
            DataQuery<ProductoUsoDto> result;

            var resultService = await _service.GetByIdAsync(new Guid(id));

            if (resultService != null)
            {
                result = HelperStatus.ResponseHelper(_mapper.Map<ProductoUsoDto>(resultService), HttpStatusCode.OK);
                return Ok(result);
            }
        ...
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(DataQuery<ProductoUsoDto>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] PostProductoUsoDto dto)
        {
            if (dto == null) return BadRequest(HelperStatus.ResponseHelper(false, HttpStatusCode.BadRequest, Messages.Result_BadRequest));
            DataQuery<ProductoUsoDto> result;
            var data = _mapper.Map<ProductoUso>(dto);
            ...
            if (resultService)
            {
                ...
                result = HelperStatus.ResponseHelper(_mapper.Map<ProductoUsoDto>(dataCrearted), HttpStatusCode.Created, Messages.Result_Created);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(string id, [FromBody] PutProductoUsoDto dto)
        {
            ...
            var dataDto = _mapper.Map<ProductoUso>(dto);
