...
namespace Warehouse.Sol.Web.Controllers
{
    ...
    public class laboratoryController : BaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<List<LaboratorioDto>>))]
        ...
        {
            DataQuery<List<LaboratorioDto>> result;

            var list = await _service.GetAllAsync(filterDto);
            if (list != null)
            {
                result = HelperStatus.ResponseHelper(this._mapper.Map<List<LaboratorioDto>>(list), HttpStatusCode.OK);
                return Ok(result);
            }
            result = HelperStatus.ResponseHelper(new List<LaboratorioDto>(), HttpStatusCode.NotFound, Messages.Result_Empty);
            return NotFound(result);
        }

        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DataQuery<LaboratorioDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(DataQuery<object>))]
        ...
        public async Task<ActionResult> GetById(string id)
        {
            ...
            DataQuery<LaboratorioDto> result;

            var resultService = await _service.GetByIdAsync(new Guid(id));

            if (resultService != null)
            {
                result = HelperStatus.ResponseHelper(_mapper.Map<LaboratorioDto>(resultService), HttpStatusCode.OK);
                return Ok(result);
            }
        ...
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(DataQuery<LaboratorioDto>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] PostLaboratorioDto dto)
        {
            if (dto == null) return BadRequest(HelperStatus.ResponseHelper(false, HttpStatusCode.BadRequest, Messages.Result_BadRequest));
            DataQuery<LaboratorioDto> result;
            var data = _mapper.Map<Laboratorio>(dto);

            var resultService = await _service.CrateAsync(data);
            if (resultService)
            {
                var providerCrearted = await _service.GetByIdAsync(data.Id);

                result = HelperStatus.ResponseHelper(_mapper.Map<LaboratorioDto>(providerCrearted), HttpStatusCode.Created, Messages.Result_Created);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(DataQuery<bool>))]
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(string id, [FromBody] PutLaboratorioDto dto)
        {
            ...
            var dataDto = _mapper.Map<Laboratorio>(dto);
